...
int WINAPI WinMain( HINSTANCE hThisInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int nCmdShow )
{
    ...
    hMainWin = CreateWindowEx( 0,           // Extended possibilites for variation
...
LRESULT CALLBACK WindowProcedure( HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_CREATE:
        ...
        // "Play" button
        CreateWindowW( L"button", L"Play", WS_VISIBLE | WS_CHILD, 155, 10, 50, 25, hwnd, (HMENU)3, NULL, NULL );

        // "Stop" button
        CreateWindowW( L"button", L"Stop", WS_VISIBLE | WS_CHILD, 210, 10, 50, 25, hwnd, (HMENU)4, NULL, NULL );

        // Back step button
        CreateWindowW( L"button", L"<<", WS_VISIBLE | WS_CHILD, 265, 10, 50, 25, hwnd, (HMENU)5, NULL, NULL );

        // Forward step button
        CreateWindowW( L"button", L">>", WS_VISIBLE | WS_CHILD, 320, 10, 50, 25, hwnd, (HMENU)6, NULL, NULL );
        ...
        // Timesig text box
        hTime
            = CreateWindowW( L"static", NULL, WS_VISIBLE | WS_CHILD | SS_CENTER, 100, 45, 50, 25, hwnd, (HMENU)8, NULL, NULL );

        // Tempo text box
        hTempo
            = CreateWindowW( L"static", NULL, WS_VISIBLE | WS_CHILD | SS_CENTER, 155, 45, 50, 25, hwnd, (HMENU)8, NULL, NULL );

        // Meas - beat text box
        hMeas
            = CreateWindowW( L"static", NULL, WS_VISIBLE | WS_CHILD | SS_CENTER, 210, 45, 50, 25, hwnd, (HMENU)7, NULL, NULL );

        // SMPTE text box
        hSmpte
            = CreateWindowW( L"static", NULL, WS_VISIBLE | WS_CHILD | SS_CENTER, 265, 45, 105, 25, hwnd, (HMENU)7, NULL, NULL );

        // Marker text box
        hMarker
            = CreateWindowW( L"static", NULL, WS_VISIBLE | WS_CHILD | SS_LEFT, 400, 45, 380, 25, hwnd, (HMENU)7, NULL, NULL );
        ...
        for ( int i = 0; i < 16; i++ )
        {
            ...
            */
            ...
            CreateWindowW( L"static", s, WS_VISIBLE | WS_CHILD | SS_LEFT, 10, 80 + 30 * i, 30, 25, hwnd, NULL, NULL, NULL );
    ...
    { // Shows the file choosing control and waits until it is closed
        ...
        {   // File loading OK
            ...
            if ( sequencer->GetMultiTrack()->GetNumTracksWithEvents() == 1 ) // all events in one track: format 0
            {
                ...
                sequencer->GetMultiTrack()->AssignEventsToTracks( sequencer->GetMultiTrack()->GetTrack( 0 ) );
...
const char *GetSmpteString()
{
    ...
    */
    ...
    sprintf( s, "%d:%02d:%02d:%02d", smpte.GetHours(), smpte.GetMinutes(), smpte.GetSeconds(), smpte.GetFrames() );
    return s;
}

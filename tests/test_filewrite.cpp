...
class TestMIDIFileEvents : public MIDIFileEvents
{
  public:
    ...
    void mf_starttrack(int trk) override { _start_track_count++; }

    void mf_endtrack(int trk) override { _end_track_count++; }

    void mf_eot(MIDIClockTime time) override { _eot_count++; }

    void mf_error(char const* msg) override { _error_count++; }
...
TEST_CASE("MIDIFileWrite meta event writing")
{
    ...
    SUBCASE("Text meta event")
    {
        ...
        size_t expected_size =
            8 + 1 + 1 + 1 + 1 + strlen(text);  // track header + delta + FF + type + len + text
    ...
    SUBCASE("MIDI file with note sequence")
    {
        ...
        for (size_t i = 22; i < data.size() - 3; ++i) {
            if (data[i] == 0x90 && data[i + 1] == 60 &&
                data[i + 2] == 100) {  // Note on C4, vel 100
                found_note_on = true;
            }
...
TEST_CASE("MIDIFileWrite error handling")
{
    ...
    SUBCASE("Large data handling")
    {
        ...
        CHECK(
            note_on_count >
            50);  // Should have substantial note events (running status reduces count)

...
public:
    ...
    {
        _note_on_count++;
        _note_events.push_back({msg.get_time(), msg.get_channel(), msg.get_note(), msg.get_velocity()});
    }
    ...
    {
        _control_count++;
        _control_events.push_back({msg.get_time(), msg.get_channel(), msg.get_controller(), msg.get_controller_value()});
    }
    ...
    struct NoteEvent { MIDIClockTime time; int channel; int note; int velocity; };
    struct ControlEvent { MIDIClockTime time; int channel; int controller; int value; };
    struct ProgramEvent { MIDIClockTime time; int channel; int program; };
...
TEST_CASE("MIDIFileRead error handling")
{
    ...
    {
        std::vector<std::uint8_t> truncated_data = {'M', 'T', 'h', 'd', 0x00, 0x00};  // Missing header length and data
...
TEST_CASE("MIDIFileRead format variations")
{
    ...
    SUBCASE("Different time divisions")
    {
        auto test_division = [](int division) {
            ...
            data.insert(data.end(), {static_cast<std::uint8_t>((division >> 8) & 0xFF), 
...
TEST_CASE("MIDIFileRead meta event parsing")
{
    ...
    SUBCASE("Time signature meta event")
    {
        ...
        data.insert(data.end(), {0x00, 0xFF, 0x58, 0x04});  // Delta 0, time sig meta event, length 4
